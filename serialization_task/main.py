...
class Widget():
    ...
    def to_binary(self):
        ...
        children_data = b"".join([child.to_binary()
    ...
    def from_binary(cls, binary_data, parent=None):
        ...
        current_pos = 4
        class_name = binary_data[current_pos:current_pos +
                                 class_name_length].decode()
        current_pos += class_name_length

        property_length = struct.unpack(
            "i", binary_data[current_pos:current_pos + 4])[0]
        current_pos += 4
        property_value = binary_data[current_pos:current_pos +
                                     property_length].decode()
        ...
        while cursor < children_length:
            child_node, child_cursor = root_element.from_binary(
                children_data[cursor:], parent=root_element)
            cursor += child_cursor
